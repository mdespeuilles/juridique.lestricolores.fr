<template>
  <div>
    ...
    <div v-for="actionnaire in submission.actionnaires">
      <p>
    ...
    <p>
      Il existe entre les propriétaires des actions ci-après créées et de toutes
      celles qui le seraient ultérieurement, une société par actions simplifiée
      régie par les lois et règlements en vigueur, et par les présents statuts.
    </p>
    ...
    <p>
      ...
      tous établissements, fonds de commerce, usines, ateliers, se rapportant à
      l'une ou l'autre des activités spécifiées ci-dessus ;
    </p>
    <p>
      - la prise, l'acquisition, l'exploitation ou la cession de tous procédés,
      brevets et droits de propriété intellectuelle concernant ces activités ;
    </p>
    <p>
      - la participation, directe ou indirecte, de la Société dans toutes
      opérations financières, mobilières ou immobilières ou entreprises
      commerciales ou industrielles pouvant se rattacher à l'objet social ou à
    ...
    <p>
      La dénomination sociale est :	{{ submission.raisonSociale }}
    </p>

    <p>
      Dans tous les actes et documents émanant de la Société et destinés aux
      tiers, la dénomination doit être précédée ou suivie immédiatement des mots
      écrits lisiblement "<em>Société par actions simplifiée</em>" ou des
      initiales "<em>SAS</em>" et de l'énonciation du montant du capital social.
    </p>
    <p>
    ...
    <p>
      <em>Transfert par le Président</em>
    </p>
    <p>
      Il peut être transféré en tout endroit par simple décision du Président qui
      est habilité à modifier les statuts en conséquence.
    </p>
    <p>
      <em><u>Transfert par les associés</u></em>
    </p>
    <p>
      Il peut être transféré en tout endroit par décision collective des associés
      prise dans les conditions de l’article 25 des présents statuts.
    </p>

    <h2>Article 5 – Durée</h2>

    <p>
      La durée de la Société est fixée à <strong>QUATRE-VINGT-DIX-NEUF</strong>
      (99) années à compter de la date de son immatriculation au Registre du
      commerce et des sociétés, sauf les cas de dissolution anticipée ou de
      prorogation.
    </p>
    ...
    <p>
      Une somme en numéraire de {{ submission|capitalNumeraire|numberToLetter }} EUROS ({{ submission|capitalNumeraire|currency }}),
      correspondant à <strong>{{ submission|capitalNumeraire|numberToLetter }}</strong> ({{ submission|capitalNumeraire }}) actions de numéraire, d'une
      valeur nominale de <strong>1 EUROS</strong> (1 €) chacune,
      souscrites en totalité et intégralement libérées, ainsi qu'il résulte du
      certificat établi par la banque.
    </p>

    <p>
      La somme totale versée par les associés, soit <strong>{{ submission|capitalNumeraire|numberToLetter }} EUROS</strong>
      ({{ submission|capitalNumeraire|currency }}), a été régulièrement déposée à un compte ouvert au nom de la
      Société en formation, à ladite banque.
    </p>
    ...
    <div v-else>
      <p>
        ...
        euros ({{ submission|capitalNumeraire|currency }}) a été dès avant ce jour, déposée sur un compte ouvert au nom de la société en formation.
      </p>
      <p>
        Elle sera retirée par la Gérance sur présentation du certificat du Greffe du Tribunal de commerce attestant l'immatriculation de la Société au Registre du Commerce et des Sociétés.
      </p>
    ...
    <div v-for="actionnaire in submission.actionnaires" v-if="actionnaire.apports && actionnaire.apports[0].valorisation">
      <p>
    ...
    <div v-if="submission.verseCreation.value === 1">
      ...
      <p>Les soussignés déclarent que toutes les actions représentant le capital social leur appartiennent, sont réparties entre eux dans les proportions indiquées ci-dessus correspondant à leurs apports respectifs et qu'elles sont toutes souscrites et libérées comme indiqué ci-dessus.</p>
    </div>
    ...
    <p>
      <u>Article 9 – Représentation des parts sociales</u>
    </p>
    <p>
